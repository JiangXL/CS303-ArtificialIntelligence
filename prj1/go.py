...
class AI(object):
    ...
    def go(self, chessboard):
        ...
        if(len(idx)>0): # if no position, just stop
            new_pos = self.descision(chessboard)
        else:
            print("No empty position")
        #==============Find new pos========================================
    ...
    def descision(self, chessboard):
        print("Sloving...")
        print(self.color)
        print(chessboard)
        
        pattern_scan(self. chessboard)
        choose_pos=(1,1)
        return choose_pos
    
    # Scan the pattern
    def pattern_scan(self, chessboard):
        print("Pattern Scan")
        box=[[0,0],[chessboard_size-1, chessboard_size-1]] # box we need to scan
        cadination 
        return cadination
